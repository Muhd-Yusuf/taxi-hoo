    ...
    <div className="min-h-full bg-zinc-50 pb-6">
      ...
      <div className="px-5 pt-5 max-w-3xl mx-auto">
        ...
        <motion.div
          ...
          className="bg-brand-500 rounded-2xl overflow-hidden shadow-xl relative"
        >
          <div className="p-5">
            ...
            <button className="w-full h-12 bg-white text-brand-600 font-semibold rounded-xl mt-5 text-sm active:scale-[0.98] transition-transform">
              Cash Out to Bank
            </button>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-3 gap-3 mt-5">
          {[
            { label: "Today", value: `P${(mockEarnings.today / 1000).toFixed(1)}k`, textColor: "text-brand-600" },
            ...
            {mockEarnings.weeklyData.map((d, i) => {
              ...
              return (
                <div key={d.day} className="flex-1 flex flex-col items-center gap-2">
                  ...
                  />
                  <span className={`text-[11px] font-medium ${isToday ? "text-brand-600" : "text-zinc-400"}`}>
                    {d.day.slice(0, 2)}
                  </span>
                </div>
              );
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 p-5 mt-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[11px] text-brand-600 font-medium">View all</span>
          </div>
          <div className="space-y-0">
            {transactions.map((tx, i) => (
              ...
              >
                ...
                <p className={`text-sm font-bold flex-shrink-0 ${tx.positive ? "text-brand-600" : "text-red-500"}`}>
                  {tx.amount}
                </p>
              </div>
            ))}
